...
4. GitHub Release (`softprops/action-gh-release`):
   - `tag_name` = `VERSION`;
   - **заголовок релиза = заголовок PR**;
   - **тело релиза = тело PR**; если пусто — заголовок PR;
...
- **Повторный запуск успешного `release.yml`** — падение на существующем теге;
  осознанно: повторная публикация той же версии бессмысленна.
