...
- **Два мержа в один день** — версии различаются временем HHMM (и хешем), коллизий нет.
- **Повторный запуск** — версия детерминирована коммитом (дата и время HHMM из коммита), поэтому перезапуск в любой день указывает на ту же версию: `action-gh-release` обновляет существующий релиз и перезаливает артефакт (не падает).
